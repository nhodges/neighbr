<?php
	...
	if(!isset($_SESSION['loggedin']) OR $_SESSION['loggedin'] == FALSE) { header("Location: http://neighbr.net"); }
	...
	switch($_GET['type']) {
		case "text";
			...
			if(!$_POST['texttitle'] OR !$_POST['texttext']) { die(header("Location: http://neighbr.net/")); }
			$sql = "INSERT INTO posts (neighbr, type, title, note) VALUES ('$_SESSION[username]', 'text', '$_POST[texttitle]', '$text')";
			$result = $db->query($sql);
			echo ($result) ? header("Location: http://neighbr.net/$_SESSION[username]/") : "We couldn't share your note! :(";
			break;
		case "image";
			if(!$_POST['imagetext'] or !$_FILES['imagefile']['name']) { die(header("Location: http://neighbr.net/")); }
			$name = $_FILES['imagefile']['name'];
			$source = "http://neighbr.net/images/$name";
			if(is_uploaded_file($_FILES['imagefile']['tmp_name'])) {
				...
				if(move_uploaded_file($tmp, "images/$name")) {
					$sql = "INSERT INTO posts (neighbr, type, source, title, note) VALUES ('$_SESSION[username]', 'image', 'http://neighbr.net/images/$name', '$name', '$_POST[imagetext]')";  
					$result = $db->query($sql);
					echo ($result) ? header("Location: http://neighbr.net/$_SESSION[username]/") : "We couldn't share your image! :(";
				} else {
				...
				*/			
				...
				die(header("Location: http://neighbr.net/"));
			}
			break;
		case "video";
			if(!$_POST['videosource']) { die(header("Location: http://neighbr.net/")); } else { require_once "./include/inurtube.php"; }
		...
		case "code";
			...
			if(!$_POST['codetitle'] OR !$_POST['codetext']) { die(header("Location: http://neighbr.net/")); }
			$sql = "INSERT INTO posts (neighbr, type, title, note) VALUES ('$_SESSION[username]', 'code', '$_POST[codetitle]', '$code')";
			$result = $db->query($sql);
			echo ($result) ? header("Location: http://neighbr.net/$_SESSION[username]/") : "We couldn't share your code! :(";
			break;
		case "comment";
			$comment = strip_tags($_POST[commenttext]);
			if(!$_POST['commenttitle'] OR !$_POST['commenttext']) { die(header("Location: http://neighbr.net/view/$_POST[postid]")); }
			$sql = "INSERT INTO comments (neighbr, post, title, note) VALUES ('$_SESSION[username]', $_POST[postid], '$_POST[commenttitle]', '$comment')";
			$result = $db->query($sql);
			echo ($result) ? header("Location: http://neighbr.net/view/$_POST[postid]") : "We couldn't share your note! :(";
			break;
		default;
