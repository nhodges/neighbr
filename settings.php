		...
		foreach($xml->channel->item as $pic) {
			...
			if(mysql_num_rows($result) == 0) {
				...
				if(fwrite($handle, $data)) {
					fclose($handle);
					$query = "INSERT INTO posts (neighbr, type, source, title, note, timestamp) VALUES ('" . $_SESSION['username'] . "', 'image', 'http://neighbr.net/images/" . $name . "', '" . $name . "', '" . $note . "', '" . date("Y-m-d H:i:s", strtotime($pic->pubDate)) . "')";
